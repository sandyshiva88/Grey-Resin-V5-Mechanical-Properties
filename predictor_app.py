...
# Prediction function
def predict_all_models(layer_thickness, curing_temp, curing_time,
                       weight_gain_uts, weight_gain_fs, weight_gain_charpy, weight_gain_izod,
                       uts_before, fs_before, charpy_before, izod_before):

    # Define model info and assign respective weight gains
    targets = {
        "UTS After 72h (MPa)": ("UTS Before (MPa)", df_UTS, uts_before, weight_gain_uts),
        "Flexural Strength After 72h (MPa)": ("Flexural Strength Before (MPa)", df_FS, fs_before, weight_gain_fs),
        "Impact Strength (Charpy) After 72h (kJ/m2)": ("Impact Strength (Charpy) Before (kJ/m2)", df_IS_Charpy, charpy_before, weight_gain_charpy),
        "Impact Strength (Izod) After 72h (kJ/m2)": ("Impact Strength (Izod) Before (kJ/m2)", df_IS_Izod, izod_before, weight_gain_izod)
    }

    results = {}
    for target_name, (before_col, df, before_value, weight_gain) in targets.items():
...
# Streamlit UI
st.title("SLA 3D Printed Material Property Predictor for Grey Resin V5 Developed by Sandeep Kumar, PhD Scholar, Mechanical Engineering, IIT Ropar, Punjab - 140001")
...
if st.sidebar.button("Predict Properties After 72h"):
    with st.spinner("Predicting..."):
        results = predict_all_models(layer_thickness, curing_temp, curing_time,
                                     weight_gain_uts, weight_gain_fs, weight_gain_charpy, weight_gain_izod,
                                     uts_before, fs_before, charpy_before, izod_before)
